...
point_t random_point_triangle(const triangle_t& tri)
{
  ...
  const float c1 = tri.C.x() * r1sqr * (r2 * tri.C.x() + b1) + a1;
  const float c2 = tri.C.y() * r1sqr * (r2 * tri.C.y() + b2) + a2;
  const float c3 = tri.C.z() * r1sqr * (r2 * tri.C.z() + b3) + a3;
