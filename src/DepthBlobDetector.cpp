...
{
#ifdef DEBUG_PARAMS
  cout << "UNKNOWN PIXEL VALUE: " << m_unknown_pixel_value << endl;
#endif
}
...
void DepthBlobDetector::findBlobs(cv::Mat binary_image, cv::Mat orig_image, cv::Mat mask_image, std::vector<Blob>& ret_blobs) const
{
  ...
  for (size_t contourIdx = 0; contourIdx < contours.size(); contourIdx++)
  {
...
#endif
    {
      ...
      const double ratio = 4 * CV_PI * blob.area / (perimeter * perimeter);
...
#ifndef DISABLE_PARALLELIZATION
